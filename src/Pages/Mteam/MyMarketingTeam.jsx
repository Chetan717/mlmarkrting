...
function TeamLeads({ member, data, loading, error, onBack }) {
  ...
  return (
    <div style={{ maxWidth: 1400, margin: "0 auto" }}>
      ...
      {loading ? <Empty title="Loading sanitized users…" text="Mobile number and password are excluded by the server." /> : <div style={{ overflowX: "auto", border: "1px solid var(--p-border)", borderRadius: 14 }}><table style={{ width: "100%", minWidth: 1040, borderCollapse: "collapse", background: "var(--p-card)" }}><thead><tr style={{ background: "var(--p-card2)" }}>{["#", "User", "Joined", "Last Download", "MLM Profile", "Company", "Plan", "Plan Status", "Expiry", "Days Left", "Lead Status", "Next Follow-up"].map(title => <th key={title} style={th}>{title}</th>)}</tr></thead><tbody>{visible.map((lead, index) => <tr key={lead.id} style={{ borderTop: "1px solid var(--p-border)" }}><td style={td}>{(safePage - 1) * PAGE_SIZE + index + 1}</td><td style={{ ...td, fontWeight: 800, color: "var(--p-text)" }}>{lead.name || "User"}</td><td style={td}>{dateText(lead.joinedAt)}</td><td style={{ ...td, color: lead.lastDownloadAt ? "#10b981" : "var(--p-text-4)", fontWeight: lead.lastDownloadAt ? 700 : 500 }}>{formatLastDownload(lead.lastDownloadAt)}</td><td style={td}><Badge value={lead.hasMlmProfile ? "Yes" : "No"} good={lead.hasMlmProfile} /></td><td style={td}>{lead.companyName || "—"}</td><td style={td}>{lead.plan || "—"}</td><td style={td}><Badge value={lead.planStatus} good={lead.planStatus === "Active"} /></td><td style={td}>{lead.expiryDate || "—"}</td><td style={td}>{lead.daysLeft ?? "—"}</td><td style={td}>{lead.leadStatus || "New"}</td><td style={td}>{lead.nextFollowupDate || "—"}</td></tr>)}{!visible.length && <tr><td colSpan="12" style={{ ...td, textAlign: "center", padding: 40 }}>No users match these filters.</td></tr>}</tbody></table></div>}
